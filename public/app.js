...
function renderMemoList(memos) {
  ...
  for (const memo of memos) {
    ...
    item.innerHTML = `
      ...
      ${memo.image_url ? `<img class="memo-image" src="${escapeHtml(memo.image_url)}" alt="" />` : ""}
    `;
...
formEl.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (editingId) {
      await updateMemo(editingId, { title, content });
      resetEditMode();
    } else {
    ...
    formEl.reset();
    await loadMemos();
    setStatus(isEditing ? "수정했습니다." : "저장했습니다.");
  } catch (err) {
    setStatus(err.message);
  }
});
